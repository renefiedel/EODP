 ...
 */
   ...
   local_status=xv_swath_pos_run(&run_id, &stf_data, 
   ...
   local_status = xv_zone_vis_time_run(&run_id,
   ...
   local_status=xv_time_segments_not_run(&run_id,
   ...
   local_status=xv_time_segments_or_run(&run_id,
                                        &orbit_type, &order_switch,
   ...
   local_status=xv_time_segments_and_run(&run_id,
                                         &orbit_type, &order_switch,
                                         &number_segments,                                  /* input segments list 1*/
                                         bgn_orbit, bgn_second, bgn_microsec, bgn_cycle,
                                         end_orbit, end_second, end_microsec, end_cycle,
   ...
   local_status=xv_time_segments_sort_run(&run_id,
   ...
   local_status=xv_time_segments_merge_run(&run_id,
                                           &orbit_type, &order_switch,
                                           &number_segments,                     /* input segments list 1*/
                                           bgn_orbit, bgn_second, bgn_microsec, bgn_cycle,
                                           end_orbit, end_second, end_microsec, end_cycle,
...
*/
  ...
  /* DELTA */
  local_status=xv_time_segments_delta_run(&run_id,
                                          &orbit_type,
  ...
  /* zone 5*/
   ...
   local_status=xv_multizones_vis_time_run(&run_id, &orbit_type, 
  ...
  local_status = xv_station_vis_time_run( &run_id,
                                          &orbit_type, 
                                          &start_orbit, &start_cycle,
                                          &stop_orbit, &stop_cycle,
  ...
  local_status = xv_multistations_vis_time_run(&run_id,
                                               &orbit_type, 
                                               &start_orbit, &start_cycle,
                                               &stop_orbit, &stop_cycle,
                                               &swath_flag, swath_file,
...
*/
   ...
   printf("\n\n\nXV_TIME_SEGMENTS_MAPPING\n");
   local_status = xv_time_segments_mapping_run(&run_id,
                                               &orbit_type, &start_orbit, &start_cycle, &stop_orbit, &stop_cycle,
  ...
  /*call to orbit_info */
  local_status = xo_orbit_info_run(&run_id,
                                   &orbit,
  ...
  /* call to orbit_extra */
  local_status=xv_orbit_extra_run(&run_id,
                                  &orbit,
                                  orbit_info_vector,
                                  &num_sza, sza,
                                  /*outputs*/
  ...
  printf("\n\nInputs: ");   
  printf("\n   Absolute Orbit: %d", abs_orbit);   
  printf("\n  Sun Zenit angles: %gº, %gº", sza[0], sza[1]);
  printf("\nOutputs: ");   
  printf("\n   SZA up - down: %gº --> %f - %f", sza[0], sza_up[0], sza_down[0]);
  printf("\n                  %gº --> %f - %f", sza[1], sza_up[1], sza_down[1]);
   ...
   *-----------------------------------*/
   ...
   local_status = xv_gen_swath_run(&run_id,
                                   &req_orbit, sdf_name, 
                                   dir_name, stf_name,
                                   file_class,
                                   &version_number,
   ...
   local_status = xv_gen_swath_no_file_run(&run_id, &req_orbit, &sdf, 
                                           &stf, ierr);
   xd_free_sdf(&sdf);
   ...
   local_status = xv_zone_vis_time_no_file_run(&run_id, &orbit_type, 
                                               &start_orbit, &start_cycle,
                                               &stop_orbit, &stop_cycle,
                                               &stf_data, &zone_data,
                                               &projection, &zone_num,
  ...
  /*get visibility segments */
  local_status = xv_station_vis_time_no_file_run( &run_id, &orbit_type, 
                                                  &start_orbit, &start_cycle,
                                                  &stop_orbit, &stop_cycle,
  ...
  /* Create run id for source satellite */
  local_status =  xl_run_init(&sat_id1, &time_id, &model_id, &run_id1, xl_ierr);
  if (status == XL_ERR)
  {
    func_id = XL_RUN_INIT_ID;
    xl_get_msg(&func_id, ierr, &n, msg);
  ...
  status =  xo_run_init(&run_id1, &orbit_id1, ierr);
  if (status == XO_ERR)
  {
    func_id = XO_RUN_INIT_ID;
    xo_get_msg(&func_id, ierr, &n, msg);
  ...
  status = xp_run_init(&run_id1, &sat_nom_trans_id1,
                       &sat_trans_id1, &instr_trans_id1,
                       &atmos_id, &dem_id, ierr);
  if (status == XP_ERR)
  {
    func_id = XP_RUN_INIT_ID;
    xp_get_msg(&func_id, ierr, &n, msg);
    xp_print_msg(&n, msg);
  }
   
   /* Create run id for target satellite */
  local_status =  xl_run_init(&sat_id2, &time_id, &model_id, &run_id2, xl_ierr);
  if (status == XL_ERR)
  {
    func_id = XL_RUN_INIT_ID;
    xl_get_msg(&func_id, ierr, &n, msg);
  ...
  status =  xo_run_init(&run_id2, &orbit_id2, ierr);
  if (status == XO_ERR)
  {
    func_id = XO_RUN_INIT_ID;
    xo_get_msg(&func_id, ierr, &n, msg);
  ...
  status = xp_run_init(&run_id2, &sat_nom_trans_id2,
  ...
  */
  ...
  /* Call xv_sc_vis_time */
  local_status = xv_sc_vis_time_run(&run_id1, &run_id2,
                                    &orbit_type,
  ...
  /* Close IDs */
  /* --------- */
  xp_run_close(&run_id1);
  xo_run_close(&run_id1);
  xl_run_close(&run_id1);
  
  xp_run_close(&run_id2);
  xo_run_close(&run_id2);
  xl_run_close(&run_id2);
