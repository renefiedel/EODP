...
/******************************************************************************/
  ...
  /* Call xl_run_init function */
  /* ------------------------- */
  status = xl_run_init(&sat_id, &time_id, &model_id, &run_id, xl_ierr);
  ...
  {
    n = 0;
    sprintf(msg[n++], "\nRun Id. could not be initialized");
    xl_print_msg(&n, msg);
  } 
  else
  {
    printf("\nRun Id. correctly initialized");
  }
...
/******************************************************************************/
  ...
  /* ------------------------------------------------------------------------ */
   ...
   ext_status = xl_change_cart_cs_run(&run_id, &calc_mode, &cs_in_t, &cs_out_t,
  ...
  /* ------------------------------------------------------------------------ */
   ...
   ext_status = xl_cart_to_geod_run(&run_id, &calc_mode, rr_t, rrd_t, &lon_t, &lat_t, &h_t,
                                    &lond_t, &latd_t, &hd_t);
  ...
  /* ------------------------------------------------------------------------ */
   ...
   ext_status = xl_geod_to_cart_run(&run_id, &calc_mode, &lon_t, &lat_t, &h_t,
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_cart_to_kepl_run(&run_id, pos_in, vel_in, &kepl_mode, kepl_out, xl_ierr );
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_kepl_to_cart_run(&run_id, &kepl_mode, kepl_in, pos_out, vel_out, xl_ierr );
...
/******************************************************************************/
  ...
  /* ------------------------------------------------------------------------ */
   ...
   ext_status = xl_geod_distance_run(&run_id, &lon1_t, &lat1_t, &lon2_t, &lat2_t,
                                     &h_t, &d_t, &az1_t, &az2_t);
...
/******************************************************************************/
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_sun_run(&run_id, &time_ref, &time_ut1, rsun, rdsun, &local_err);
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_moon_run(&run_id, &time_ref, &time_ut1, rmoon, rdmoon, &local_err);
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_planet_run(&run_id, &planet_id, &time_ref, &time_ut1, rplanet, rdplanet, &local_err);
   ...
   printf("\n- rplanet[0]       : %lf ", rplanet[0] );   /* Planet position vector in Earth Fixed.0 CS (x,y,z) [m] */
   printf("\n- rplanet[1]       : %lf ", rplanet[1] );
   printf("\n- rplanet[2]       : %lf ", rplanet[2] );
   printf("\n- rdplanet[0]      : %lf ", rdplanet[0] );  /* Planet velocity vector in Earth Fixed.0 CS (x,y,z) [m/s] */
  ...
  /* ------------------------------------------------------------------------ */
   ...
   status = xl_star_radec_run(&run_id, &time_ref, &time_ut1, &ra0, &dec0, &mu_ra, &mu_dec,
   ...
   printf("\n- ra               : %lf ", ra );   /* Star right ascension in Mean of Date at UT1 [deg] */
   printf("\n- dec              : %lf ", dec );  /* Star declination in Mean of Date at UT1 [deg] */
...
/******************************************************************************/
  ...
  /* ---------------------------------------- */
  status = xl_time_transport_to_ascii_run(&run_id,  &t2a_trans_id_in,  &t2a_time_ref_in,
  ...
  /* -------------------------------------------- */
  status = xl_time_transport_to_transport_run(&run_id, &t2t_trans_id_in,
  ...
  /* --------------------------------------------- */
  status = xl_time_transport_to_processing_run(&run_id, &t2p_trans_id_in,
  ...
  /* ----------------------------------------- */
  status = xl_time_processing_to_ascii_run(&run_id,  &p2a_proc_id_in,  &p2a_time_ref_in,
  ...
  /* -------------------------------------------- */
  status = xl_time_processing_to_transport_run(&run_id, &p2t_proc_id_in,
  ...
  /* ---------------------------------------------- */
  status = xl_time_processing_to_processing_run(&run_id, &p2p_proc_id_in,
  ...
  /* ------------------------------------ */
  status = xl_time_ascii_to_ascii_run(&run_id,  &a2a_ascii_id_in,  &a2a_time_ref_in,
  ...
  /* ---------------------------------------- */
  status = xl_time_ascii_to_transport_run(&run_id,  &a2t_ascii_id_in,  &a2t_time_ref_in,
  ...
  /* ----------------------------------------- */
  status = xl_time_ascii_to_processing_run(&run_id,  &a2p_ascii_id_in,  &a2p_time_ref_in,
...
/******************************************************************************/
  ...
  /* Call xl_run_init function with the new satellite */
  /* ------------------------------------------------ */
  status = xl_run_init(&sat_id, &time_id, &model_id, &run_id_2, xl_ierr);
  ...
  /* --------------------------------- */
  status = xl_time_obt_to_time_run(&run_id_2, &proc_id, &envisat_obt_param, 
  ...
  /* --------------------------------- */
  status = xl_time_time_to_obt_run(&run_id_2, &proc_id, &envisat_obt_param, 
  ...
  /* ------------------------------------- */
   ...
   status = xl_position_on_orbit_run(&run_id,&angle_type,&time_ref,&poo_time,
  ...
  /* ------------------------------------- */
   ...
   status = xl_cart_to_radec_run(&run_id, &mode, &cs_in, pos, vel, 
                                 &ra, &dec,
                                 &mu_ra, &mu_dec, 
                                 &rad_vel, &par, xl_ierr);
  ...
  /* ------------------------------------- */
   ...
   status = xl_radec_to_cart_run(&run_id, &mode, &cs_in, &ra, &dec,
  ...
  /* -------------------------------------------------------------------- */
  /* Function(s): xl_star_catalog_run                                       */
  /* -------------------------------------------------------------------- */
   ...
   status = xl_topocentric_to_ef_run(&run_id, &mode, &deriv, pos, vel, 
                                     &azim, &elev, &range,
                                     &azim_d, &elev_d, &range_d, 
   ...
   status = xl_ef_to_topocentric_run(&run_id, &mode, &deriv, pos, vel,
                                     ef_dir, ef_dir_d,
                                     &azim, &elev, &range,
                                     &azim_d, &elev_d, &range_d,                           
                                     xl_ierr);
   ...
   /* Call xl_run_close function */
   /* --------------------------- */
   xl_run_close(&run_id);
   xl_run_close(&run_id_2);
