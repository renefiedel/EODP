 ...
 *                    - xp_target_inter_run
 *                    - xp_target_ground_range_run
 *                    - xp_target_incidence_angle_run
 *                    - xp_target_range_run
 *                    - xp_target_range_rate_run
 *                    - xp_target_tangent_run
 *                    - xp_target_altitude_run
 *                    - xp_target_star_run
 *                    - xp_target_tangent_sun_run
 *                    - xp_target_tangent_moon_run
 *                    - xp_target_generic_run
 *                    - xp_target_station_run
...
/******************************************************************************/
  ...
  /* Time references initialization                                           */
  /* ------------------------------------------------------------------------ */
  /* Function(s): xl_time_ref_init, xl_run_init                               */
  /* ------------------------------------------------------------------------ */
  ...
  status = xl_run_init(&sat_id, &time_id, &model_id, &run_id_2, ierr);
  sat_id = XP_SAT_GENERIC;
  status = xl_run_init(&sat_id, &time_id, &model_id, &run_id_3, ierr);
  ...
  status = xo_run_init(&run_id_3, &orbit_id, ierr);
  if (status < XL_OK)
  {
    n = 0;
    sprintf(msg[n++], "\nRun Id. could not be initialized");
...
/******************************************************************************/
  ...
  printf("\n- Init Status      : %li",xp_sat_nominal_att_init_status(&sat_nom_trans_id));
  printf("\n- Init Mode        : %li",xp_sat_nominal_att_get_mode(&sat_nom_trans_id));
  printf("\n- Model Enum       : %li",model_enum);
  printf("\n- Model Param      : [%12.9lf,%12.9lf,%12.9lf]",model_param[0], model_param[1], model_param[2]);
...
/******************************************************************************/
  ...
  status = xp_run_init(&run_id_3, &sat_nom_trans_id,
                       &sat_trans_id, &instr_trans_id,
                       &atmos_id, &dem_id, ierr);
  if (status == XL_ERR)
  {
  ...
  /* ------------------------------------------------------------------------ */
  /* Function(s): xp_attitude_compute_run                                     */
  /* ------------------------------------------------------------------------ */

  printf("\n\n\nxp_attitude_compute_run\n");
  ...
  /* --------------------------------- */
  status = xp_attitude_compute_run(&run_id, &attitude_id_2,
                                   &time_ref, &time,
                                   pos_2, vel_2, acc_2, 
                                   &target_frame,
                                   /* output */
  ...
  target_type = XP_SAT_ATT;
  status = xp_gen_attitude_data_run(&run_id_3, 
                                    &target_type,
                                    &time_interval,
                                    &time_step,
                                    &data_type, 
  ...
  status = xp_gen_attitude_file_run(&run_id_3, 
                                    &target_type,
                                    &time_interval,
                                    &time_step,
                                    &data_type, 
...
/******************************************************************************/
  ...
  /* Function(s): xp_run_init                                                 */
  /* ------------------------------------------------------------------------ */
  status = xp_run_init(&run_id_2, &sat_nom_trans_id,
                       &sat_trans_id, &instr_trans_id,
                       &atmos_id, &dem_id, ierr);
  ...
  /* Call xp_attitude_compute function */
  /* ----------------------------- */
  status = xp_attitude_compute_run(&run_id_2,
                                   &attitude_id,
                                   &time_ref, &time,
                                   pos, vel, acc, 
                                   &target_frame,
  ...
  /* ----------------------------- */
  status =  xp_target_inter_run(&run_id_2, &attitude_id,
  ...
  /* ------------------------------------ */
  status = xp_target_ground_range_run(&run_id_2, &attitude_id,
                                      &deriv, &los_az, &los_el, 
                                      &geod_alt, &distance, &los_az_rate, &los_el_rate,
                                      /* output */
                                      &num_user_target, &num_los_target,
                                      &target_id, ierr);
  ...
  /* --------------------------------------- */
  status = xp_target_incidence_angle_run(&run_id_2,
                                         &attitude_id,
  ...
  /* ----------------------------- */
  status = xp_target_range_run(&run_id_2, 
                               &attitude_id,
                               &deriv, &los_az,
  ...
  /* ---------------------------------- */
  status = xp_target_range_rate_run(&run_id_2, &attitude_id,
                                    &deriv, &ef_range_rate,
  ...
  /* ------------------------------- */
  status = xp_target_tangent_run(&run_id_2,
                                 &attitude_id,
                                 &deriv, &los_az,
  ...
  /* -------------------------------- */
  status = xp_target_altitude_run(&run_id_2,
                                  &attitude_id,
                                  &deriv, &los_az,
  ...
  /* ---------------------------- */
  status = xp_target_star_run(&run_id_2,
                              &attitude_id_2,
  ...
  /* --------------------- */
  status = xp_target_tangent_moon_run(&run_id_2, 
                                      &attitude_id,
                                      &deriv, &iray, &freq,
                                      /* output */
                                      &num_user_target, &num_los_target,
  ...
  /* ------------------------------- */
  status = xp_target_station_run(&run_id_2,
                                 &attitude_id_2,
                                 &deriv, &geoc_long,
  ...
  /* Call xp_change_frame_run function */
  /* --------------------------------- */
  status = xp_change_frame_run (&run_id,&cf_mode,
  ...
  /* ------------------------------------------------------------------------ */
  /* Function(s): xp_run_close, xl_run_close                                  */
  /* ------------------------------------------------------------------------ */
  printf("\n\nxp_run_close\n");

  xp_run_close(&run_id);
  xp_run_close(&run_id_2);
  xp_run_close(&run_id_3);

  xo_run_close(&run_id_3);

  xl_run_close(&run_id);
  xl_run_close(&run_id_2);
  xl_run_close(&run_id_3);
